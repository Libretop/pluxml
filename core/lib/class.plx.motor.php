...
class plxMotor {
	...
	public function getartId($url='') {					
		foreach($this->plxGlob_arts->aFiles as $key=>$value) {			
			if (strpos($value,$url)) {
				return $key;
			}
		}
	...
	public function getcatId($url='') {						
		foreach($this->aCats as $key=>$value) {				
			if ($value['url']==$url) {				
				return $key;
			}
		}
	...
	public function getstatId($url='') {								
		foreach($this->aStats as $key=>$value) {				
			if ($value['url']==$url) {				
				return $key;
			}
		}
	...
	public function prechauffage() {
		...
		elseif($this->get AND preg_match('/^article([0-9]+)\/?([a-z0-9-]+)?/',$this->get,$capture)) {
			...
			$capt = ($capture[1]==0)? $this->getartId($capture[2]) : $capture[1];			
			$this->cible = str_pad($capt,4,'0',STR_PAD_LEFT); # On complete sur 4 caracteres
		...
		elseif($this->get AND preg_match('/^categorie([0-9]+)\/?([a-z0-9-]+)?/',$this->get,$capture)) {
			$capt = ($capture[1]==0)? $this->getcatId($capture[2]) : $capture[1];
			$this->cible = str_pad($capt,3,'0',STR_PAD_LEFT); # On complete sur 3 caracteres
		...
		elseif($this->get AND preg_match('/^([a-z0-9-]+)/',$this->get,$capture)) {
			$capt = $this->getstatId($capture[1]);
			if ($capt) {
			$this->cible = str_pad($capt,3,'0',STR_PAD_LEFT); # On complète sur 3 caractères
			if(!isset($this->aStats[$this->cible]) OR !$this->aStats[$this->cible]['active']) {
				$this->error404(L_UNKNOWN_STATIC);
			} else {
				if(!empty($this->aConf['homestatic']) AND $capture[1]){
				...
				if($this->aStats[$this->cible]['url']==$capture[1]) {
					$this->mode = 'static'; # Mode static
					$this->template = $this->aStats[$this->cible]['template'];
				} else {
					$this->redir301($this->urlRewrite('?static'.intval($this->cible).'/'.$this->aStats[$this->cible]['url']));
	...
	public function addart($name, $desc, $keywords, $subject, $text, $cat='002', $author='002') {	
	$art['title'] = trim($name);
	$art['url']=$text;
	$art['chapo'] = '';
	$art['content'] =  trim($subject);
	$art['tags'] = $text;
	$art['author']  = '002';	
	$art['date_creation_year'] = date('Y');
	$art['date_creation_month'] = date('m');
	$art['date_creation_day'] = date('d');
	$art['date_creation_time']= date('H:i');	
	$art['catId'] = $cat;
	$art['numero'] = '0000';
	$art['template'] = 'article.php';
	$art['meta_description']=$desc;
	$art['meta_keywords']=$keywords;
	$this->addArticle($art,'');		
	}
	...
	public function addArticle($content, $id) {

		if($id == '0000' OR $id == '')
			$id = $this->nextIdArticle();

		if(trim($content['url']) == '')
			$content['url'] = plxUtils::title2url(plxUtils::translit($content['title']));
		else
		...
		$filename = PLX_ROOT.$this->aConf['racine_articles'].$id.'.'.$content['catId'].'.'.trim($content['author']).'.'.$time.'.'.$content['url'].'.xml';		
		$suc = plxUtils::write($xml,$filename);
