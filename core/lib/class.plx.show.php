...
class plxShow {
	...
	public function mainTitle($type='') {
		...
		if($type == 'string') return $title;
		if($type == 'strong') {
			$words = explode (' ',$title);
			$slogun[0] = implode(' ',array_slice($words,0,2));
			$slogun[1] = implode(' ',array_slice($words,2));
			return $slogun;
		}
	...
	public function mainlink($output='') {
		if ($output!=='string') {
			echo $this->plxMotor->urlRewrite();
		} else {
			return $this->plxMotor->urlRewrite();
		}
	}
	
	public function post($code='') {				
		$this->plxMotor->redir301($this->plxMotor->urlRewrite().'post/'.trim($code));
	}
	...
	public function subTitle($type='') {
		$desc = plxUtils::strCheck($this->plxMotor->aConf['description']);
		if($type == 'strong') {
			$words = explode (' ',$desc);
			$slogun[0] = implode(' ',array_slice($words,0,4));
			$slogun[1] = implode(' ',array_slice($words,4,4));
			$slogun[2] = implode(' ',array_slice($words,8));
			return $slogun;
		}
		if($type == 'string') return $desc; 
		echo $desc;
	}
			...
			foreach($this->plxMotor->aCats as $k=>$v) {
				...
				if($in AND !$ex) {
					if(($v['articles']>0 OR $this->plxMotor->aConf['display_empty_cat']) AND ($v['menu']=='oui') AND $v['active']) { # On a des articles
						...
						$name = str_replace('#cat_status',($this->catId()==intval($k)?'is-active':'noactive'), $name);
	...
	public function articleUrl($echo=true, $extra='') {

		# On affiche l'URL
		$id = intval($this->plxMotor->plxRecord_arts->f('numero'));
		$url = $this->plxMotor->urlRewrite('test/'.$this->plxMotor->plxRecord_arts->f('url').$extra);
		if($echo)
			echo $url;
		else
			return $url;
	 ...
	 * @author	Stephane F
	 **/
	public function artAuthorEmail($output='') {

		if(isset($this->plxMotor->aUsers[$this->plxMotor->plxRecord_arts->f('author')]['email'])) {			
			if ($output=='string') {
				return plxUtils::strCheck($this->plxMotor->aUsers[$this->plxMotor->plxRecord_arts->f('author')]['email']);
			} else {
	...
	public function AdminEmail($output='') {
		if ($output!=='string') {		
			echo $this->plxMotor->aUsers['001']['email'];				
		}
	}
	...
	**/
	 ...
	 * @author	Stephane F
	 **/
	public function staticList($extra='', $format='<li class="#static_class #static_status" id="#static_id" ><a href="#static_url" title="#static_name">#static_name</a></li>', $format_group='<span class="#group_class #group_status">#group_name</span>', $menublog=false, $selector=false) {
		...
		if($this->plxMotor->aStats) {
			foreach($this->plxMotor->aStats as $k=>$v) {
				if($v['active'] == 1 AND $v['menu'] == 'oui') { # La page  est bien active et dispo ds le menu
					...
					if ($selector) { 
						$stat = str_replace('#static_class',$v['meta_keywords'],$stat);
	...
	public function staticCible() {
		return str_pad($this->staticId(),3,'0',STR_PAD_LEFT);
	}
	 ...
	 * @author	Florent MONTHEL
	 **/
	public function staticTitle($id) {
		if ($id) {			
			echo plxUtils::strCheck($this->plxMotor->aStats[(string)$id]['name']);
		} else {
			echo plxUtils::strCheck($this->plxMotor->aStats[ $this->plxMotor->cible ]['name']);
		}
	}
	
	public function staticLink($id) {
		if ($id) {			
			echo $this->mainlink().plxUtils::strCheck($this->plxMotor->aStats[(string)$id]['url']);
		} else {
			echo plxUtils::strCheck($this->plxMotor->aStats[ $this->plxMotor->cible ]['url']);
		}
	}
	
	public function catTitle($id) {
		echo plxUtils::strCheck($this->plxMotor->aCats[(string)$id]['name']);
	}
	
	public function catLink($id) {
		echo $this->mainlink().'section/'.plxUtils::strCheck($this->plxMotor->aCats[(string)$id]['url']);
	}
	...
	public function staticContent($id) {
		...
		if ($id) {						
				...
				$addon = $this->plxMotor->cible;
			}
		# On va verifier que la page a inclure est lisible
		if($this->plxMotor->aStats[ $addon ]['readable'] == 1) {						
			# On genere le nom du fichier a inclure
			$file = PLX_ROOT.$this->plxMotor->aConf['racine_statiques'].$addon;
			$file .= '.'.$this->plxMotor->aStats[ $addon ]['url'].'.php';
